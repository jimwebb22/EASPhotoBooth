...
public final class ImageProcessingViewModel: ObservableObject {
    ...
    /// Auto-tune pointCount from the image's tonal coverage on next process.
    private var pendingAutoPointCount = false
    /// Settings the current/last pipeline run used — skips redundant
    /// reprocessing (including the one triggered by auto-tuning pointCount).
    private var lastProcessedSettings: DrawingSettings?
    ...
    public func reprocessDebounced() {
        ...
        debounceTask = Task {
            ...
            // Skip no-op reprocessing (e.g. the settings change generated by
            // auto-tuning pointCount inside the pipeline itself).
            guard settings != lastProcessedSettings else { return }
            await processImage()
        }
    }
    ...
    public func processImage() async {
        ...
        processingTask = Task {
            ...
            do {
                ...
                // Auto-tune point count from the image's tonal coverage on
                // first processing of a new photo.
                if pendingAutoPointCount {
                    pendingAutoPointCount = false
                    let auto = DrawingSettings.adaptivePointCount(
                        meanDensity: preprocessed.densityMap.meanDensity
                    )
                    localSettings.pointCount = auto
                    settings.pointCount = auto
                }
                lastProcessedSettings = localSettings
